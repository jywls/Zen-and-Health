// UntieRope.js - High-Fidelity 3D Physical Rope Puzzle
...
class UntieRopeGame {
  ...
  startGame() {
    ...
    // Ensure Cannon is loaded
    if (typeof CANNON === 'undefined') {
        console.error("CANNON is not defined. Physics will not work.");
        return;
    }
  ...
  createRope(name, p1, p2, colorName) {
    ...
    for (let i = 0; i <= segmentCount; i++) {
        ...
        if (i > 0) {
            ...
            if (prev.mesh.physicsImpostor && sphere.physicsImpostor) {
                const joint = new BABYLON.DistanceJoint({ 
                    distance: BABYLON.Vector3.Distance(prev.mesh.position, sphere.position) * 0.95 
                });
