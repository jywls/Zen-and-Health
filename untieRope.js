// UntieRope.js - 3D Physical Rope Puzzle with Win Detection
...
class UntieRopeGame {
  ...
  startGame() {
    ...
    new BABYLON.HemisphericLight('ambient', new BABYLON.Vector3(0, 1, 0), this.scene).intensity = 0.7;
  ...
  createRope(name, p1, p2, colorName) {
    ...
    for (let i = 0; i <= segmentCount; i++) {
        ...
        if (i > 0) {
            ...
            const joint = new BABYLON.DistanceJoint({ distance: BABYLON.Vector3.Distance(prev.mesh.position, sphere.position) * 0.95 });
            prev.mesh.physicsImpostor.addJoint(sphere.physicsImpostor, joint);
        }
    }
  ...
  checkWin() {
    // Check for intersections between line segments connecting the pins
    let intersections = 0;
    for (let i = 0; i < this.ropes.length; i++) {
        for (let j = i + 1; j < this.ropes.length; j++) {
            if (this.doIntersect(this.ropes[i].p1.position, this.ropes[i].p2.position, this.ropes[j].p1.position, this.ropes[j].p2.position)) {
                intersections++;
            }
        }
    }
    ...
    if (intersections === 0) {
        ...
        if (typeof showVictoryModal === 'function') {
            showVictoryModal(`You untied the ropes in ${this.moves} moves!`, `Moves: ${this.moves}`, () => {
                this.level++;
  ...
  // Helper for 2D intersection check on the board plane
  doIntersect(p1, q1, p2, q2) {
    const onSegment = (p, q, r) => q.x <= Math.max(p.x, r.x) && q.x >= Math.min(p.x, r.x) && q.z <= Math.max(p.z, r.z) && q.z >= Math.min(p.z, r.z);
